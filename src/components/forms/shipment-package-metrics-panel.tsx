...
export type { PackageInputMode } from "@/lib/shipment-metrics";
...
type PackageMetricsPanelProps<T extends FieldValues> = {
  ...
  onPackageInputModeChange?: (mode: PackageInputMode) => void;
...
export function PackageMetricsPanel<T extends FieldValues>({
  ...
  packageCount,
  weight,
  packageInputMode: controlledMode,
  onPackageInputModeChange,
  onWeightChange,
  idPrefix = "",
  showContainerDetails = true,
}: PackageMetricsPanelProps<T>) {
  const [internalMode, setInternalMode] = useState<PackageInputMode>("dimensions");
  const packageInputMode = controlledMode ?? internalMode;

  function setPackageInputMode(mode: PackageInputMode) {
    if (controlledMode === undefined) {
      setInternalMode(mode);
    }
    onPackageInputModeChange?.(mode);
  }
  ...
  const metrics = useMemo(() => {
    if (packageInputMode !== "dimensions") {
      return null;
    }
  ...
  }, [packageInputMode, lengthCm, widthCm, heightCm, packageCount, containerType]);

  useEffect(() => {
    if (packageInputMode === "dimensions" && metrics && onWeightChange) {
      onWeightChange(metrics.volumetricWeightKg);
    }
  }, [packageInputMode, metrics, onWeightChange]);
    ...
    <div className="space-y-3 rounded-lg border border-border p-3">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Enter weight directly, or provide dimensions to calculate volumetric weight.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1 rounded-lg border bg-muted/40 p-1">
        <button
          ...
          className={cn(
            ...
            packageInputMode === "weight"
              ? "bg-background shadow-sm"
              : "text-muted-foreground hover:text-foreground"
          )}
          onClick={() => setPackageInputMode("weight")}
        >
          Weight only
        </button>
        <button
          ...
          onClick={() => setPackageInputMode("dimensions")}
        >
          Dimensions
        </button>
      </div>
      ...
      {packageInputMode === "weight" ? (
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex flex-col gap-2">
            ...
            <Input
              ...
              min="0"
              placeholder="e.g. 25"
              {...register("weight" as Path<T>, { valueAsNumber: true })}
            />
            {errors.weight && (
              <p className="text-sm text-destructive">{String(errors.weight.message)}</p>
            )}
          </div>
          <div className="flex flex-col gap-2">
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-2">
              <Label htmlFor={`${idPrefix}packageCount-dim`}>Package Count</Label>
              <Input
                id={`${idPrefix}packageCount-dim`}
                type="number"
                step="1"
                min="1"
                defaultValue={idPrefix ? undefined : 1}
          ...
          <input type="hidden" {...register("weight" as Path<T>, { valueAsNumber: true })} />
          {weight != null && weight > 0 && (
            <p className="text-sm text-muted-foreground">
              Billable weight: {weight.toFixed(2)} kg
            </p>
          )}
        </>
      )}
